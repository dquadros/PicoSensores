...
void loop() {
  ...
  // Pega o novo estado e acrescenta ao histórico
  uint8_t states = (received & STATES_MASK) >> 28;
  history = ((history & 0x3F) << 2) | CURR_STATE(states);

  // Testa padrões de movimento para frente e para trás
  if (history == move_up) {
    if (val < 100) {
      val++;
      Serial.println(val);
...
// Prepara o programa da PIO
void encoder_program_init(uint enc_pin_a, uint enc_pin_b) {
  // Carrega o programa e escolhe a máquina de estado
