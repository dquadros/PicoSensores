 ...
 * @brief Exemplo com o Sensor BMP280/BME280
 * @version 1.0
 * @date 2023-06-23
 * 
...
// Classe para acessar o sensor BME280/BMP280
class BMx280 {
...
BMx280::BMx280 (i2c_inst_t *i2c, uint8_t addr,
  ...
  // Le os coeficientes de calibracao
  readCalibCoeff();
...
// Muda o modo do sensor
// programa o oversampling
...
// Dispara uma medicao
// Se bWait = true, espera o resultado
void BMx280::doMeasure(bool bWait) {
  ...
  if (bWait) {
    sleep_ms(1);  // espera o status mudar
    while (!getRawData()) {
      sleep_ms(1);
    }
  }
...
// Pega o dado bruto, se disponivel
// retorna true se tinha dado disponivel
bool BMx280::getRawData() {
  if (read8(STATUS) & STATUS_MEASURING) {
    return false; // medicao nao concluida ainda
  } else {
...
float BMx280::getPressure() {
  if (getRawData() && (up != 0x80000)) {
    ...
    if (var1 == 0)
      return NAN; // evita divisao por zero
...
float BMx280::getHumidity() {
  if (id == BME280_ID) {
    ...
    return NAN; // BMP280 nao mede umidade
  }
}

// Calcula a altitude a partir da pressao
float BMx280::calcAltitude(float pressure, float sealevelPressure) {
  return 44330.0 * (1.0 - pow(pressure/sealevelPressure, 0.1903));
}

// Returna a "fine resolution temperature"
// (como definida na secao 8.2 do datasheet)
...
// Le e salva os coeficientes de calibracao
...
// Le um valor de 8 bits de um registrador
...
// Le um valor de 20 bits de um registrador
...
int main() {
  ...
  while(1) {
    ...
    printf("Temperatura: %.1f C  Pressao: %.0f Pa  Altitude: %.1f m",
      temp, pressure, altitude);
    if (bmp.getDeviceId() == BMx280::BME280_ID) {
      float humidity = bmp.getHumidity();
      printf("  Umidade: %.1f %%", humidity);
    }
    printf("\n");
  }
}
